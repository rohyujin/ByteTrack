...
from yolox.evaluators import MOTEvaluatorCustom as MOTEvaluator
...
def make_parser():
    ...
    parser.add_argument("--custom", dest="custom", default=False, action="store_true", help="test custom.")
    return parser
...
def main(exp, args, num_gpu):
    ...
    *_, summary = evaluator.evaluate(
        model, is_distributed, args.fp16, trt_file, decoder, exp.test_size, results_folder, face_eye_dict,
    )
    logger.info("\n" + summary)
    ...
    if args.mot20:
        ...
        gtfiles = glob.glob(os.path.join('datasets/kw_video/', '*/labels_with_ids/*.txt'))
    # else:
